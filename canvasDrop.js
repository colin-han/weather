...
var Drop = function () {
    ...
    // if (randomEdge > 1) {
        this.pos = new Vector(50 + Math.random() * (canvas.width + 100), -80);
    // } else {
...
var Bounce = function (x, y, radius) {

    var dist = Math.random() * 7;
    var angle = Math.PI + Math.random() * Math.PI;

    this.pos = new Vector(x, y);
    this.radius = 0.2 + Math.random() * 0.4 * radius;
    this.vel = new Vector(
        Math.cos(angle) * dist,
        Math.sin(angle) * dist
    );
    ...
    while (i--) {
        ...
        if (drop.pos.y >= canvas.height) {
            ...
            if (OPTS.hasBounce) {
                ...
                while (n--)
                    bounces.push(new Bounce(drop.pos.x, canvas.height, drop.radius));
            }
            //如果drop实例下降到底部，则需要在drops数组中清楚该实例对象
            drops.splice(i, 1);
        }
...
function setStyle(){
    if(OPTS.type =="rain"){
        ctx.lineWidth = 1 * DPR;
        ctx.strokeStyle = 'rgba(223,223,223,0.4)';
        ctx.fillStyle = 'rgba(223,223,223,0.4)';
    }else{
        ctx.lineWidth = 2 * DPR;
